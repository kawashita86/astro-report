...
- source_spec: `_bmad-output/implementation-artifacts/spec-9-5-a-stage-track-i-can-watch-and-a-gate-failure-i-can-read.md`
  summary: The run stage-view poll-error banner shows "Connessione persa — nuovo tentativo…" for every failed poll, including a permanent 404 (run deleted) or 500, and the 2s poll never backs off or gives up.
  evidence: Blind-hunter review of the Story 9.5 diff. Story 9.5's spec deliberately deferred the 5s/15s backoff + manual "Riprova" ladder to Story 9.8's network-error state primitive (its I/O & Edge-Case Matrix says so). Distinguishing a retryable network blip from a terminal error, and stopping the poll on the latter, belongs with that same 9.8 work.
  triage: defer (blind-hunter review of spec-9-5-a-stage-track-i-can-watch-and-a-gate-failure-i-can-read, review-loop 0)

- source_spec: `_bmad-output/implementation-artifacts/spec-9-5-a-stage-track-i-can-watch-and-a-gate-failure-i-can-read.md`
  summary: The stage-track's decorative connector (`<span class="stage-track__connector">` in `report_run_poll.html`) renders as a direct child of `<ol class="stage-track">`, sitting between `<li>` siblings — invalid per the HTML5 `<ol>` content model (li / script-supporting elements only), which undercuts the Design Notes' own stated reason for choosing `<ol>` ("gives screen-reader users position info for free").
  evidence: Blind-hunter review of the Story 9.5 review-loop-1 diff. A correct fix (CSS `::before` pseudo-element per node, or restructuring the connector inside each `<li>`) has to be reconciled with `.stage-track`'s `flex-wrap: wrap` behavior (a naive absolutely-positioned connector would misplace itself across a wrapped row boundary), so it is nontrivial CSS rework, not a one-line fix. Story 9.5's own CSS comments name Story 9.8 as the consolidation point for this exact component ("do not build on these class names elsewhere") — that is where this belongs.
  triage: defer (blind-hunter review of spec-9-5-a-stage-track-i-can-watch-and-a-gate-failure-i-can-read, review-loop 1)

- source_spec: `_bmad-output/implementation-artifacts/spec-9-5-a-stage-track-i-can-watch-and-a-gate-failure-i-can-read.md`
  summary: The stage-track active-dot's pulse `box-shadow` (`@keyframes stage-track-pulse`) uses `--primary-100`, which has no dark-theme override anywhere in `tokens.css` — the same light-lavender value renders against a dark background in dark mode.
  evidence: Blind-hunter review of the Story 9.5 review-loop-1 diff. `--primary-100` is an established, theme-invariant token already reused this way pre-Story-9.5 (e.g. an existing focus-ring `box-shadow` in `tokens.css`), so this is a pre-existing app-wide token pattern surfaced incidentally by the new pulse animation, not something Story 9.5 introduced on its own — it decorates the already-compliant active-dot colour, it does not replace it as the state signal.
  triage: defer (blind-hunter review of spec-9-5-a-stage-track-i-can-watch-and-a-gate-failure-i-can-read, review-loop 1)

- source_spec: `_bmad-output/implementation-artifacts/spec-9-6-the-report-reading-sheet-and-the-payload-view-made-readable.md`
  summary: Click-to-copy `.badge-mono` chips (report/payload entry ids, draft violation-card ids, dashboard month codes) have no accessible name describing the copy action and no `aria-live` announcement of the "Copiato" success state, so a screen-reader user hears only the raw identifier with no cue that activating it copies anything or that it worked.
  evidence: Blind-hunter review of Story 9.6's diff. `shell.js`'s new delegated click-to-copy listener and the `button.badge-mono` markup carry no `aria-label`/`aria-description` and no `aria-live` region; the "Copiato" feedback is a plain `textContent` swap, visual-only. Epic 9's own context designates Story 9.9 as the cross-cutting "Italian sweep + accessibility floor" audit/gap-fill pass over every screen from 9.1-9.8, which is the natural home for this.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-9-6-report-run-breadcrumb.md`
  summary: The new `_report_run_breadcrumb.html` partial (and its `_client_tabs.html` precedent) renders as a bare `<p class="breadcrumb">` with plain `/` text separators rather than a `<nav aria-label="…">`/`<ol>` breadcrumb landmark, so a screen reader gets no "breadcrumb" navigation semantics.
  evidence: Blind-hunter review of this story's diff. Matches `_client_tabs.html`'s own pre-existing breadcrumb markup exactly (this story intentionally mirrored it for consistency), so fixing one without the other would diverge the two breadcrumbs' structure -- a `tokens.css` comment on `.breadcrumb` already flags it "PROVISIONAL," naming Story 9.8 as the intended consolidation point for shared component partials like this one.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-5-7-accept-a-gate-violation-after-review-so-a-report-can-complete.md`
  summary: A Report closed via accepted Gate violations (Story 5.7) shows only a bare "Superato con N eccezioni" count everywhere it's named; the per-violation `gate_violation_review` rows (kind, section, sentence, detail, cited entries) this story persists specifically for audit purposes are never surfaced back through any page or link, so Francesco (or a future SM-7 hand-sampler) has no way to see *what* was actually accepted on a given Report after the fact.
  evidence: Blind-hunter review of this story's diff. `shell/adapters/postgres/gate_violation_review.py`'s own docstring frames the denormalized columns as existing "so this row stands alone as an audit record" -- that value is currently unrealized in the UI. Out of this story's approved scope (epics.md's Story 5.7 ACs only require the count badge), but a real gap for the audit use case the table was built for.

- source_spec: `_bmad-output/planning-artifacts/sprint-change-proposal-2026-09-14.md`
  summary: The PRD states three different figures for the monthly Report ceiling in the same NFR section -- "100–200 per month" (Throughput and latency), "30–200 Reports per month" (Cost), and the architecture spine's own Deferred section shorthands it to a flat "200-reports-per-month ceiling" -- none reconciled.
  evidence: Blind-hunter review flagged this while reviewing the 2026-09-14 correct-course batch; pre-existing across `prd.md` and `ARCHITECTURE-SPINE.md`, not introduced by this batch's AD-20 amendment (which only repeats the spine's existing "200" shorthand). Needs a PRD pass to pick one number and correct the others, not a one-shot fix.

- source_spec: `_bmad-output/planning-artifacts/sprint-change-proposal-2026-09-14.md`
  summary: AD-20's amendment for `REPORT_RUN_MODE=background` never specifies the in-process scheduler's own tick interval -- `poll` mode's cadence is pinned precisely (`every 2s`, plus the 1s/2s poll-failure backoff), but the scheduler's polling frequency is left completely undefined.
  evidence: Blind-hunter review of the 2026-09-14 correct-course batch. Deliberately left open at the architecture-amendment level of detail (matching how `spec-3-10` pinned `advance()`'s exact one-stage-per-call contract at spec time, not architecture time) -- must be pinned by the upcoming `bmad-spec` pass on story `3-11-run-in-background-when-the-tab-closes` before implementation.

- source_spec: `_bmad-output/implementation-artifacts/spec-9-8-feedback-and-state-primitives.md`
  summary: `POLL_BACKOFF_MS_FIRST`/`POLL_BACKOFF_MS_SUBSEQUENT` in `shell/http/static/shell.js` (Story 9.8's poll-failure backoff) have no test coverage anywhere in `tests/` guarding their actual timing values.
  evidence: Blind-hunter review surfaced this while reviewing the 2026-09-14 backoff-value change (5000/15000 -> 1000/2000, `spec-reduce-poll-retry-backoff-to-1-2s.md`). The gap predates that change -- Story 9.8 shipped these constants untested originally, since there is no JS test harness in this repo (no `package.json`, no `*.test.js`) -- and the value edit itself doesn't introduce or worsen it, but nothing stops these numbers drifting silently again.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-11-run-in-background-when-the-tab-closes.md`
  summary: `GET /report-runs/{run_id}` (`shell/http/routes/report_runs.py::poll_report_run`) still 500s (unhandled `RuntimeError`) if a `ReportRun`'s `Client` row is ever actually missing, regardless of `REPORT_RUN_MODE` -- the check runs unconditionally before either mode's branch, so even a run the background scheduler has just marked `failed_at` for exactly this reason cannot be viewed through the poll route afterward.
  evidence: Blind-hunter review-loop 2 on this story's diff. Pre-existing since Story 3.5/3.10 (the check and its `raise RuntimeError` are unchanged by this story) and, in normal operation, believed unreachable -- Client deletion cascades its `ReportRun` rows, so a live run should never outlive its Client. Real only under a data-integrity violation (manual DB surgery, a missed cascade edge case), matching the same "raise `RuntimeError` on an impossible state" convention already used elsewhere in this file (`_load_passed_report_bundle`); not something this story's scope covers fixing.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-gate-skip-regen-low-violations.md`
  summary: Story 5.4's original first Acceptance Criteria block ("the regeneration count for the run is incremented") and FR-21's top-line requirement/first consequence bullet ("Regeneration is automatic and bounded.") were left unqualified by the 2026-09-17 amendment, so they now read as contradicting the new AC/bullet directly beneath them; separately, "configured ceiling"/"configured low-violation ceiling" (used in the architecture spine, AD-10 mirror, and Story 5.4 AC) reads as user/env-configurable and contradicts `driver.py`'s own comment on `_MIN_VIOLATIONS_FOR_AUTO_REGENERATION`, which explicitly says "non-runtime-configurable."
  evidence: Both issues are inside doc diffs Francesco approved verbatim on 2026-09-17 (sprint-change-proposal-2026-09-17.md §4); fixing them means editing already-approved planning-doc text, which is outside this pass's authorized scope. Worth a small wording pass in a future correct-course.

- source_spec: `_bmad-output/implementation-artifacts/spec-gate-skip-regen-low-violations.md`
  summary: A run whose Gate failure is short-circuited by the new low-violation path never increments `regeneration_count`, so SM-C2 (regeneration count, the PRD's counter-metric to SM-5's Gate pass rate) can no longer distinguish "never had a Gate problem" from "had a Gate problem too small to warrant a regeneration" -- the new path is a real Gate-failure signal that SM-C2 will now systematically undercount.
  evidence: Sprint-change-proposal-2026-09-17.md's Impact Analysis did not revisit SM-5/SM-C2; no PRD text notes this new metric blind spot. A product-analytics question, not a code correctness gap.
