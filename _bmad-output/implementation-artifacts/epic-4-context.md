...
Style Guide and a purely-derived summary of prior months (`ReportTheme`), and returns cited sentences
rather than free prose. This is the highest-risk epic in the whole build — one deliverable (the Style
Guide) is writing work no engineering can substitute for, and the product's output quality rests on it.
...
- Register: professional, warm, non-fatalistic, addressing the reader as an adult; no sentence predicts
  ...
  the date, not vague seasonal language).
...
- A per-Section "interpretive territory" table (what each Section is about, why a given placement
  belongs to it, plus the Benessere caution) exists in the planning artifacts and should be consulted
  when writing the guide — worth double-checking at story time, since another planning note claims no
  such material exists and it must be written from scratch.
...
- Rate-limit/transient generation failures retry automatically and invisibly to Francesco, bounded,
  ...
  changing provider is a separately gated, deliberate configuration change.
- Local development runs generation against recorded responses, not the live provider, so building and
  testing this epic costs no quota and stays deterministic.
...
- **`ReportTheme` must be built before generation**, since it's a Generator port argument — building it
  after would force a breaking port change. `derive_theme(payload) -> ReportTheme` lives in
  `core/memory/`, pure and model-free, yielding dominant slow-planet Aspects by tightness, the natal
  houses of the month's Lunations, and standing retrogrades. Comparing two ReportThemes (still-active /
  tightened / resolved / new) is what makes "nothing significant changed" computed rather than judged.
  Stored in its own table, separate from generation, joining the Client-deletion cascade.
- **Exactly one Generator adapter** (Google Gemini `gemini-2.5-flash`, free tier, EEA data terms), no
  ...
  10 RPM ceiling plus run checkpointing.
...
- Domain vocabulary is fixed and untranslated (`Generator`, `StyleGuide`, `ReportTheme`, `Section`,
  `Claim`, …) — a synonym anywhere is a defect.
