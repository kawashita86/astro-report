...
def test_a_cited_day_list_entry_is_enriched_with_the_citing_sentence_text() -> None:
    ...
    # 2026-01-10 15:00 UTC -> 09:00 CST (UTC-6); day-list entries show the
    # date only, never a time-of-day (fix, 2026-09-03).
    assert fav_1["date"] == "10/01/2026"
...
def test_an_uncited_day_list_entry_still_renders_date_only_never_dropped() -> None:
    ...
    # 2026-01-18 09:00 UTC -> 03:00 CST (UTC-6); date only, no time-of-day.
    assert fav_2["date"] == "18/01/2026"

    attention = rendered["giorni_di_attenzione"][0]
    assert attention["text"] is None
    assert attention["date"] == "15/01/2026"
